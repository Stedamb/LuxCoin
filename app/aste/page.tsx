...
export default async function AuctionsPage() {
  ...
  // Fetch past auctions
  const pastAuctions = await client.fetch<Auction[]>(
    `*[_type == "auction" && endDate < now()] | order(startDate desc)`,
  );
    ...
    <main className="min-h-screen bg-background text-foreground pb-20">
      ...
      <section className="pt-32 pb-12 container mx-auto px-4 text-center">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
          Partecipa alle nostre aste esclusive. <br /> Pezzi unici, tempo
          limitato, opportunità irripetibili.
        </p>
      </section>
      ...
      <section className="container mx-auto px-4 max-w-5xl space-y-8 mb-20">
        <div className="flex items-center gap-4 mb-8">
          ...
          <span className="text-xl font-serif text-primary uppercase tracking-wider">
            Aste in Corso
          </span>
          <div className="h-px bg-white/10 flex-1" />
        </div>
        ...
        {upcomingAuctions.length > 0 ? (
          upcomingAuctions.map((auction) => (
            <Card
              key={auction._id}
              className="bg-card border-none overflow-hidden hover:bg-card/80 transition-all group shadow-lg"
            >
                    ...
                    <div className="flex items-center justify-end w-full xl:w-auto gap-6">
                      {auction.link ? (
                        <Button
                          asChild
                          variant="premium"
                          className="h-12 px-6 w-full xl:w-auto"
                        >
                          <Link
                            href={auction.link}
                            target="_blank"
                        ...
                        <Button
                          disabled
                          variant="outline"
                          className="h-12 px-6 w-full xl:w-auto border-white/10 text-muted-foreground"
                        >
          ...
          <div className="text-center py-12 bg-card/30 rounded-lg border border-white/5">
            <p className="text-muted-foreground text-lg mb-2">
              Non ci sono aste attive al momento.
            </p>
            <p className="text-sm text-muted-foreground/60">
              Controlla più tardi o visualizza l'archivio delle aste passate.
            </p>
          </div>
        )}
      </section>
      ...
      {pastAuctions.length > 0 && (
        <section className="container mx-auto px-4 max-w-5xl space-y-8">
          <div className="flex items-center gap-4 mb-8">
            <div className="h-px bg-white/10 flex-1" />
            <span className="text-xl font-serif text-muted-foreground uppercase tracking-wider">
              Aste Concluse
            </span>
            <div className="h-px bg-white/10 flex-1" />
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 opacity-60 hover:opacity-100 transition-opacity">
            {pastAuctions.map((auction) => (
              <Card
                key={auction._id}
                className="bg-card border-none hover:bg-card/80 transition-colors"
              >
                <CardContent className="p-6 flex gap-4">
                  <div className="w-24 h-24 shrink-0 bg-black/20 rounded-md overflow-hidden relative">
                    {auction.mainImage ? (
                      <Image
                        src={urlFor(auction.mainImage)
                          .width(200)
                          .height(200)
                          .url()}
                        alt={auction.title}
                  ...
                  <div>
                    <h4 className="font-serif font-bold text-lg mb-2 text-zinc-300">
                      {auction.title}
                    </h4>
