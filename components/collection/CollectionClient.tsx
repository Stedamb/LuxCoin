...
export function CollectionClient({
  ...
  const filteredCoins = React.useMemo(() => {
    return initialCoins.filter((coin) => {
      ...
      const searchContent =
        `${coin.title} ${coin.year || ""} ${coin.category?.title || ""} ${coin.period?.title || ""}`.toLowerCase();
      ...
      const matchesCategory =
        ...
        (coin.category &&
          selectedFilters.categoryIds.includes(coin.category._id));

      const matchesPeriod =
        selectedFilters.periodIds.length === 0 ||
        (coin.period && selectedFilters.periodIds.includes(coin.period._id));

      const matchesMaterial =
        selectedFilters.materialIds.length === 0 ||
        (coin.material &&
          selectedFilters.materialIds.includes(coin.material._id));

      const matchesCondition =
        selectedFilters.conditionIds.length === 0 ||
        (coin.condition &&
          selectedFilters.conditionIds.includes(coin.condition._id));
    ...
    <div className="relative">
      ...
      <div className="flex gap-8">
        ...
        <div className="hidden lg:block">
          <FilterSidebar
            categories={itemType === "all" ? categories : []}
            periods={periods}
            materials={materials}
            conditions={conditions}
            selectedFilters={selectedFilters}
        ...
        <div className="flex-1">
          <div className="flex flex-col md:flex-row md:items-start justify-between md:mb-8 pb-8 border-b border-white/5 gap-6">
            <div className="space-y-2 text-center md:text-left">
              ...
              <p className="text-muted-foreground">
                {filteredCoins.length !== initialCoins.length
                  ? `Trovati ${filteredCoins.length} articoli su ${initialCoins.length} disponibili`
                  : `${initialCoins.length} articoli disponibili`}
              </p>
            </div>
            ...
            <div className="flex flex-col items-center md:items-end w-full max-w-sm gap-4">
              ...
              >
                ...
                {selectedFilters.categoryIds.length +
                  selectedFilters.periodIds.length +
                  selectedFilters.materialIds.length +
                  selectedFilters.conditionIds.length >
                  0 &&
          ...
          {filteredCoins.length > 0 ? (
            ...
            <div className="py-24 text-center">
              ...
              <p className="text-xl font-serif mb-2 text-foreground">
                Nessun risultato
              </p>
