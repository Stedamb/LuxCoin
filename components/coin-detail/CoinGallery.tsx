...
export function CoinGallery({
  images,
  title,
  coinGradient = "from-amber-400 to-amber-700",
}: CoinGalleryProps) {
  ...
  if (!images || images.length === 0) {
    return (
      <div className="aspect-square rounded-2xl bg-zinc-900 flex items-center justify-center border border-white/5">
        <div
          className={`w-3/4 h-3/4 rounded-full bg-linear-to-br ${coinGradient} opacity-20`}
        />
      </div>
    );
  }
            ...
            <button
              ...
              className={cn(
                ...
                current === index
                  ...
                  : "border-white/10 bg-card opacity-60 hover:opacity-30",
              )}
            >
              <Image
